...
public class FastMath {
	...
	private static double sinQ(double xa, double xb) {
		...
		final double epsilon = xa - org.apache.commons.math3.util.FastMath.EIGHTHS[idx];// idx*0.125;
	...
	public static double scalb(final double d, final int n) {
		...
		} else // we are really in the case n >= 1024
		if (exponent == 0) {
	...
	public static float scalb(final float f, final int n) {
		...
		} else // we are really in the case n >= 128
		if (exponent == 0) {
			// the input number is subnormal, normalize it
			while ((mantissa >>> 23) != 1) {
				mantissa = mantissa << 1;
